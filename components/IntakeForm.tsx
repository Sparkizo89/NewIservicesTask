...
export const IntakeForm: React.FC<IntakeFormProps> = ({ onShowToast, userId }) => {
    ...
    useEffect(() => {
        if (!userId) return;
        const docRef = doc(db, 'users', userId, 'intakeForms', 'data');
        const unsub = onSnapshot(docRef, (snap) => {
            if (snap.exists()) {
                const data = snap.data();
                if (data.forms) setSavedFormsList(data.forms);
            }
        });
        return () => unsub();
    }, [userId]);

    const saveFormsToFirestore = async (forms: any[]) => {
        if (!userId) return;
        try {
            await setDoc(doc(db, 'users', userId, 'intakeForms', 'data'), { forms });
        } catch (e) {
    ...
    const handleSave = () => {
        ...
        try {
            const savedList: any[] = [...savedFormsList];
            ...
            saveFormsToFirestore(savedList);
            setSavedFormsList(savedList);
            onShowToast(currentFormId ? "Ticket mis à jour !" : "Ticket sauvegardé !");
        } catch (error) {
    ...
    const deleteSavedForm = (id: string, e: React.MouseEvent) => {
        ...
        saveFormsToFirestore(newList);
        onShowToast("Ticket supprimé !");
    };
    ...
    const confirmSendEmail = async () => {
        ...
        const htmlBody = `
            <div style="font-family: Arial, sans-serif; font-size: 14px; color: #333; line-height: 1.6; max-w-2xl mx-auto;">
                <h2 style="color: #ea580c; border-bottom: 2px solid #ea580c; padding-bottom: 10px;">Ticket iSERVICES</h2>
                <p>Bonjour,</p>
                <p>Veuillez trouver ci-dessous le récapitulatif de votre ticket :</p>
        ...
        const payload: any = {
            ...
            cc_email: "",
            subject: `Votre Ticket - iSERVICES`,
        ...
        <div className="flex flex-col h-full overflow-y-auto scrollbar-thin px-4 pb-12 print:p-0 print:overflow-visible print:block print:h-auto">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4 print:hidden">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-tech font-bold uppercase tracking-widest text-black dark:text-white">Ticket</h1>
            ...
            <div className="hidden print:flex justify-between items-start mb-8 print:mb-2 border-b-2 border-black pb-4 print:pb-2">
                <div>
                    ...
                    <p className="text-xl print:text-sm font-sans text-neutral-800 tracking-wide mt-2 print:mt-1">Ticket</p>
                </div>
