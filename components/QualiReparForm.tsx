...
import {
    ...
    FaChartSimple, FaCalendar, FaChevronDown, FaFlask, FaEye, FaQrcode
...
const QualiReparForm: React.FC<QualiReparFormProps> = ({ onShowToast, userId }) => {
    ...
    const [qrStatus, setQrStatus] = useState<'idle' | 'waiting' | 'received'>('idle');
    ...
    // Load saved attestations from Firestore on mount
    useEffect(() => {
        if (!userId) return;
        const docRef = doc(db, 'users', userId, 'qualirepar', 'data');
        const unsub = onSnapshot(docRef, (snap) => {
            if (snap.exists()) {
                const data = snap.data();
                if (data.savedAttestations) setSavedAttestations(data.savedAttestations);
                if (data.downloadedAttestations) setDownloadedAttestations(data.downloadedAttestations);
            }
        });
    ...
    const saveToFirestore = async (newSaved: AttestationData[], newDownloaded: AttestationData[]) => {
        if (!userId) return;
        try {
            await setDoc(doc(db, 'users', userId, 'qualirepar', 'data'), {
                savedAttestations: newSaved,
                downloadedAttestations: newDownloaded
            });
        } catch (e) {
    ...
    const handleOpenQR = async () => {
        ...
        const sessionId = `sig_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
        ...
        // Create a pending session in Firestore
        await setDoc(doc(db, 'signatureSessions', sessionId), {
            status: 'pending',
            createdAt: Date.now()
        });

        // Listen via Firestore for the client's signature (works across any network)
        const unsub = onSnapshot(doc(db, 'signatureSessions', sessionId), (snap) => {
            if (!snap.exists()) return;
            const data = snap.data();
            if (data.status === 'completed' && data.signatureData) {
                // Load into canvas
                if (sigCanvas.current) {
                    sigCanvas.current.clear();
                    sigCanvas.current.fromDataURL(data.signatureData);
                }
    ...
    const handleSave = () => {
        ...
        // 4. Save to state and Firestore
        setSavedAttestations(newList);
        saveToFirestore(newList, downloadedAttestations);
    ...
    const deleteAttestation = (id: string, e: React.MouseEvent) => {
        ...
        saveToFirestore(newList, newDownloaded);
    ...
    const generatePDF = async () => {
        try {
            ...
            saveToFirestore(savedAttestations, updatedDownloaded);
    ...
    const handleDeleteDownloaded = (id: string) => {
        ...
        saveToFirestore(updatedSaved, updated);
                ...
                {showQRModal && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-sm bg-white dark:bg-[#111] border border-black/10 dark:border-white/10 rounded-3xl p-6 shadow-2xl overflow-hidden"
                        >
                            {/* Header */}
                            <div className="flex justify-between items-center mb-5">
                                <div>
                                    <h2 className="text-lg font-tech font-bold uppercase tracking-widest text-black dark:text-white flex items-center gap-2">
                                        <FaQrcode className="text-orange-500" /> Signature à distance
                                    </h2>
                                    <p className="text-xs text-neutral-500 mt-0.5">Le client scanne ce QR code avec son téléphone</p>
                                </div>
                                <button
                                    onClick={() => setShowQRModal(false)}
                            ...
                            <div className="flex flex-col items-center gap-4 mb-5">
                                ...
                                <div className="flex items-center gap-2 px-4 py-2 bg-orange-500/10 rounded-full">
                                    ...
                                    <span className="text-xs font-bold text-orange-500 uppercase tracking-wider">En attente de signature...</span>
                                </div>
                            </div>

                            {/* URL fallback */}
                            <div className="text-center">
                                <p className="text-[10px] text-neutral-400 uppercase tracking-wider mb-1">Ou accédez manuellement à l'URL :</p>
                                <p className="text-[10px] font-mono text-neutral-600 dark:text-neutral-400 break-all select-all px-2">{qrUrl}</p>
                            </div>
                        </motion.div>
                    </div>
                )}
