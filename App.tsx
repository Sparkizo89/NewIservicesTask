...
import { FaMagnifyingGlass, FaAddressBook, FaBookOpen, FaRobot, FaPaperPlane, FaSquare, FaEnvelope, FaRotate, FaChevronLeft, FaInbox, FaArrowDown, FaPaperclip, FaDownload, FaFile, FaWandMagicSparkles, FaReply, FaSpellCheck, FaArrowRightFromBracket, FaGoogle } from 'react-icons/fa6';
import { GoogleGenAI } from "@google/genai";
import { auth, db, googleProvider } from './firebase';
import { signInWithPopup, signOut, onAuthStateChanged, User } from 'firebase/auth';
import { doc, setDoc, onSnapshot, collection, updateDoc, arrayUnion, arrayRemove, getDoc } from 'firebase/firestore';
...
const MainApp: React.FC = () => {
    ...
    const handleLogin = async () => {
        try {
            ...
            handleShowToast("Erreur lors de la connexion Google");
        }
    };
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                setPinnedProcedures(data.pinnedProcedures || []);
                setLocalSentEmails(data.sentEmails || []);
            } else {
                // Initialize user document if it doesn't exist
                setDoc(docRef, { pinnedProcedures: [], sentEmails: [] });
            }
        });
    ...
    const togglePin = async (procedureId: string) => {
        if (!user) {
            handleShowToast("Connectez-vous pour épingler des procédures");
            return;
        }

        const isPinned = pinnedProcedures.includes(procedureId);
        const docRef = doc(db, 'users', user.uid);

        try {
            await updateDoc(docRef, {
                pinnedProcedures: isPinned ? arrayRemove(procedureId) : arrayUnion(procedureId)
            });
        } catch (error) {
    ...
    const handleSendReply = async () => {
        ...
        try {
            ...
            if (user) {
                const docRef = doc(db, 'users', user.uid);
                await updateDoc(docRef, {
                    sentEmails: arrayUnion(sentEmail)
                });
            } else {
                // Fallback local if not logged in (optional, but keep for consistency)
                setLocalSentEmails(prev => [sentEmail, ...prev]);
            }
    ...
    // Login gate — spinner while auth resolves, then login screen if not signed in
    if (authLoading) {
        return (
            <div className="h-screen flex items-center justify-center bg-[#0a0a0a]">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <span className="font-tech text-xs tracking-widest text-orange-500 uppercase">Chargement...</span>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="h-screen flex flex-col items-center justify-center bg-[#0a0a0a] relative overflow-hidden">
                ...
                <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-orange-600/5 rounded-full blur-[120px]"></div>
                </div>

                {/* Logo */}
                <div className="flex items-center gap-4 mb-12">
                    <div className="relative">
                        <div className="w-14 h-14 border border-[#333] bg-[#0a0a0a] flex items-center justify-center text-orange-600 rounded-full">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                        </div>
                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-orange-600 rounded-full border-2 border-[#0a0a0a]"></div>
                    </div>
                    <div>
                        <h1 className="text-4xl font-tech text-white tracking-tighter uppercase leading-none">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <p className="text-[10px] text-neutral-500 font-tech uppercase tracking-[0.2em]">OS v3.0 — Accès sécurisé</p>
                        </div>
                    </div>
                </div>

                {/* Login card */}
                <div className="relative flex flex-col items-center gap-6 p-10 rounded-2xl border border-[#1e1e1e] bg-white/[0.03] backdrop-blur-xl shadow-[0_0_60px_rgba(0,0,0,0.5)]">
                    {accessDenied ? (
                        <>
                            <div className="flex flex-col items-center gap-3 text-center">
                                <div className="w-12 h-12 rounded-full border border-red-500/30 bg-red-500/10 flex items-center justify-center text-red-400 text-xl">✕</div>
                                <p className="text-red-400 font-tech text-sm uppercase tracking-widest">Accès refusé</p>
                                <p className="text-neutral-600 text-xs max-w-[240px]">Votre adresse email n'est pas autorisée à accéder à cet espace.</p>
                            </div>
                            <button
                                onClick={() => setAccessDenied(false)}
                                className="flex items-center gap-3 px-8 py-3.5 rounded-full border border-neutral-700 bg-neutral-800/50 hover:bg-neutral-700 text-neutral-400 hover:text-white font-tech uppercase tracking-widest text-xs transition-all duration-300"
                            >
                        ...
                        <>
                            <div className="text-center">
                                <p className="text-neutral-400 font-tech text-sm uppercase tracking-widest mb-1">Authentification requise</p>
                                <p className="text-neutral-600 text-xs">Connectez-vous pour accéder à votre espace de travail</p>
                            </div>

                            <button
                                onClick={handleLogin}
                                className="flex items-center gap-3 px-8 py-3.5 rounded-full border border-orange-500/30 bg-orange-500/10 hover:bg-orange-500 hover:text-white text-orange-500 font-tech uppercase tracking-widest text-xs transition-all duration-300 shadow-[0_0_30px_rgba(234,88,12,0.15)] hover:shadow-[0_0_40px_rgba(234,88,12,0.4)] group"
                            >
                                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" className="group-hover:fill-white"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" className="group-hover:fill-white"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" className="group-hover:fill-white"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" className="group-hover:fill-white"/></svg>
                                Se connecter avec Google
                            </button>

                            <p className="text-neutral-700 text-[10px] text-center max-w-[240px] leading-relaxed">
                                Vos données sont synchronisées en temps réel et sécurisées par Firebase.
                            </p>
                        </>
                    )}
                </div>
